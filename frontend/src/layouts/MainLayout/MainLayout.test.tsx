...
it('renders the document navigation and runtime controls from the environment API', async () => {
  ...
  expect(screen.queryByRole('link', { name: '巡检能力' })).not.toBeInTheDocument()
  expect(screen.queryByRole('link', { name: '能力演进' })).not.toBeInTheDocument()
  expect(screen.queryByRole('link', { name: 'AI运行' })).not.toBeInTheDocument()
  ...
  expect(screen.getByText('AI 按需分析')).toBeInTheDocument()
